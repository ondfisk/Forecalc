...
let ``toSeq returns sequence``() =
    ...
    qt4.[1000, 1000] <- Some 11
    qt4.[10000, 1000000] <- Some 12
    qt4 |> QT4.toSeq |> Seq.toList |> should equal [ 9 ; 10 ; 11 ; 12 ]
...
let ``iteri should call all elements with index``() =
    ...
    let res = ref ""
    qt4 |> QT4.iteri (fun c r v -> res.Value <- $"c: {c}, r: {r}, v: {v}")
...
let ``mapi carries correct index``() =
    let qt4 = QT4.create<int>()
    qt4.[1512, 18243] <- Some(42)
    let result = qt4 |> QT4.mapi (fun c r v -> $"c: {c}, r: {r}, v: {v}")
...
let ``get(42, 42) calls indexer``() =
    let qt4 = QT4.create<int>()
    qt4.[42, 42] <- Some(42)
    qt4 |> QT4.get 42 42 |> should equal (Some 42)

[<Fact>]
let ``set(42, 42) calls indexer``() =
    let qt4 = QT4.create<int>()
    qt4 |> QT4.set 42 42 (Some 42)
...
let ``range (42,42) (42,42) returns elements in range``() =
    ...
    qt4.[1, 1] <- Some(1)
    qt4.[42, 42] <- Some(42)
    let result = qt4 |> QT4.range (42,42) (42,42) |> Seq.toList
