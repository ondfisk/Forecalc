...
let ``"Life, " & "the " & "Universe " & "and " & "Everything = " & 42.0 -> "LifeT, the Universe and Everything = 42"``() =
    ...
    eval cell expr workbook (HashSet<AbsCell>()) (HashSet<AbsCell>()) |> should equal (StringValue "Life, the Universe and Everything = 42")
...
let ``true&false -> "TRUEFALSE"``() =
    ...
    eval cell expr workbook (HashSet<AbsCell>()) (HashSet<AbsCell>()) |> should equal (StringValue "TRUEFALSE")
...
let ``2^8 -> 256``() =
    ...
    eval cell expr workbook (HashSet<AbsCell>()) (HashSet<AbsCell>()) |> should equal (FloatValue 256.0)
