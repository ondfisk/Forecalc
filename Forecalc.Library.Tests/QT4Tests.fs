...
[<Test>]
let ``QT4.iter should call all elements``() =
    let qt4 = QT4.create<int>() 
    qt4.[0, 0] <- Some 9
    qt4.[42, 42] <- Some 10
    ...
    let i = ref 0
    qt4 |> QT4.iter (fun x -> i := !i + x)
    !i |> should equal 42

[<Test>]
let ``QT4.toSeq returns sequence``() =
    let qt4 = QT4.create<int>() 
    qt4.[0, 0] <- Some 9
    qt4.[42, 42] <- Some 10
    qt4.[1000, 1000] <- Some 11
    qt4.[10000, 1000000] <- Some 12
    qt4 |> QT4.toSeq |> should equal (List.toSeq [ 9 ; 10 ; 11 ; 12 ])
...
[<Test>]
let ``QT4.map (*) 2 should multiply elements by 2``() =
    let qt4 = QT4.create<int>() 
    qt4.[42, 42] <- Some(21)
    let result = qt4 |> QT4.map ((*) 2)
    result.[42, 42] |> should equal (Some 42)

[<Test>]
let ``QT4.filter (fun x -> x % 2 = 0) should strip odd items``() =
    let qt4 = QT4.create<int>()
    qt4.[1, 1] <- Some(1)
    qt4.[42, 42] <- Some(42)
    let result = qt4 |> QT4.filter (fun x -> x % 2 = 0)
    result.[1, 1] |> should equal None
    result.[42, 42] |> should equal (Some 42)

[<Test>]
let ``QT4.iteri should call all elements with index``() =
    let qt4 = QT4.create<int>() 
    qt4.[1512, 18243] <- Some 42
    let res = ref ""
    qt4 |> QT4.iteri (fun c r v -> res := sprintf "c: %i, r: %i, v: %i" c r v)
    !res |> should equal "c: 1512, r: 18243, v: 42"

[<Test>]
let ``QT4.mapi carries correct index``() =
    let qt4 = QT4.create<int>() 
    qt4.[1512, 18243] <- Some(42)
    let result = qt4 |> QT4.mapi (fun c r v -> sprintf "c: %i, r: %i, v: %i" c r v)
...
[<Test>]
let ``3 element QT4.length has length 3``() =
    let qt4 = QT4.create<int>()
    qt4.[0, 0] <- Some 9
    qt4.[42, 42] <- Some 10
    qt4.[1000, 1000] <- Some 11
